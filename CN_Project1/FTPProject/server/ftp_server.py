...
def authenticate(client_socket):
    ...
    # Open and read the users.txt file to check credentials
    try:
...
def handle_file_upload(client_socket):
    ...
    with open("new" + filename, 'wb') as f:  # Prefix "new" to filename to avoid overwriting
        client_socket.send(b"Start sending the file.")
        while True:
            data = client_socket.recv(1024)
            if data.endswith(b"EOF"):  # Check for our EOF signal
                f.write(data[:-3])  # Write data excluding the EOF marker
                break
            f.write(data)
    client_socket.send(b"File uploaded successfully.\n")
    ack = client_socket.recv(1024).decode().strip()  # Wait for ACK from client
...
def handle_client(client_socket):
    try:
        ...
        # Proceed with handling commands from the client
        while True:
            ...
            if command.lower() == 'exit':
                ...
                break  # Exit the loop and proceed to close the connection
...
#         # Pass the users dictionary to handle_client
...
def start_server(port):
    ...
    try:
        while server_running:
            ...
            if server_running:
                print(f"Accepted connection from {addr}")
                # Pass the client socket to handle_client in a separate thread or as per your logic
